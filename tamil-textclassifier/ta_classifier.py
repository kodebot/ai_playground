...
Corpus = pd.read_csv("dataset/ta/news_headline_sample.csv", sep="|")

# drop empty rows
Corpus.dropna(inplace=True)
Corpus = Corpus[Corpus["label"].notnull()]
...
for index, entry in enumerate(Corpus["text"]):
    ...
    for word in words:
       # if word not in nltk.corpus.stopwords.words("tamil") and word.isalpha():
            # stemming is not done
        final_words.append(word)
    Corpus.loc[index, "text_final"] = str(final_words)

#%%
Corpus["label"].notnull().any()
Corpus["text_final"].notnull().any()
Corpus
#%%
